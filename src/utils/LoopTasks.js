	...
	async hourlyTasks() {
		...
		if (stats.guilds) {
			this.app.bot.editStatus('online', {
				name: `j.help | ${STATUS_LIST[Math.floor(Math.random() * STATUS_LIST.length)].replace('{users}', this.app.common.formatNumber(stats.users, true)).replace('{guilds}', this.app.common.formatNumber(stats.guilds, true))}`,
				type: 0
			})
		}
	}
            ...
            */
			...
			for (let i = 0; i < unhandled.data.length; i++) {
				...
				const logEmbed = new this.app.Embed()
					...
					.setDescription(`${transaction.from.name}(${transaction.from.id}) to Racoin\n\n[Link](https://dash.discoin.zws.im/#/transactions/${transaction.id}/show)`)
					.addField('Racoin Payout', `${this.app.common.formatNumber(payout)} (${this.app.common.formatNumber(refunded)} refunded)`, true)
					.addField('User', `\`\`\`\n${transaction.user}\`\`\``)
