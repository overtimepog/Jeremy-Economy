...
class Player {
	...
	async createAccount(id) {
		...
		const newPlayer = new this.app.Embed()
			.setTitle('Thanks for playing Jeremy!')
			.setColor(13451564)
        ...
        \`sell\` - Sell your items for Racoin.
        ...
        You can also use \`j.help <command>\` to see detailed command information and examples.
        ...
        Join the [support server](https://discord.gg/apKSxuE) if you need more help!`)
			.addField('Items Received', `1x ${this.app.itemdata.crate.icon}\`crate\`\nOpen it by __using__ it: \`j.use crate\`\n\nOnce you get a weapon, you can attack another player by __using__ a weapon on them: \`j.use rock @user\``)
	...
	async checkLevelXP(message, row) {
		try {
			...
			if (row.points >= xp.totalNeeded) {
				...
				if (row.level + 1 >= 5) {
					await this.app.itm.addBadge(message.author.id, 'Bot')
				}
				if (row.level + 1 >= 10) {
					await this.app.itm.addBadge(message.author.id, 'F2P')
				}
				if (row.level + 1 >= 25) {
					await this.app.itm.addBadge(message.author.id, 'P2P')
				}
				if (row.level + 1 >= 50) {
					await this.app.itm.addBadge(message.author.id, 'Genuine')
				}
				if (row.level + 1 >= 100) {
					await this.app.itm.addBadge(message.author.id, 'Valve')
				}
				if (row.level + 1 >= 200) {
					await this.app.itm.addBadge(message.author.id, 'Raccoon')
				}
