	...
	async execute(app, message, { args, prefix }) {
		...
		if (await app.cd.getCD(message.author.id, 'tradeban')) {
			...
			if (!await app.itm.hasItems(userItems, itemName, 1)) {
				...
				return message.reply(`Please enter a higher price! Minimum **${app.common.formatNumber(100)}** Racoin`)
			}
			...
			const bmEmbed = new app.Embed()
				...
				.addField('Price:', `**${app.common.formatNumber(itemCost)}** Racoin`)
				.setColor(13451564)
			...
			try {
				...
				collectorObj.collector.on('collect', async m => {
					...
					if (newAmnt && !amount) {
						...
						bmEmbed.setDescription(`Enter the price in **Racoin** for all **${amount}**:`)
						botMessage = await message.channel.createMessage(bmEmbed)
						return
					}
					else if (!amount) {
					...
					if (newCost && !price) {
						if (newCost >= 2147483647) {
							...
							return m.channel.createMessage(`Please enter a higher price! Minimum **${app.common.formatNumber(100)}** Racoin`)
						}
