...
module.exports = {
	...
	async execute(app, message, { args, prefix }) {
		...
		if (buyItem) {
			...
			if (currency === 'money') {
				const botMessage = await message.channel.createMessage(`Purchase ${buyAmount}x ${app.itemdata[buyItem].icon}\`${buyItem}\` for **${app.common.formatNumber(itemPrice * buyAmount)}** Racoin?`)
				...
				try {
					...
					if (confirmed) {
						...
						if (row.money < itemPrice * buyAmount) {
							return botMessage.edit(`You don't have enough Racoin for that purchase! You only have **${app.common.formatNumber(row.money)}**.`)
						}
			...
			const botMessage = await message.channel.createMessage(`Trade **${app.common.formatNumber(scrapPrice)}** Racoin for **${app.common.formatNumber(buyAmount, false, true)}** Scrap?`)
			...
			try {
				...
				if (confirmed) {
					...
					if (verifyRow.money < scrapPrice) {
						return botMessage.edit(`You don't have enough Racoin for that purchase! You only have **${app.common.formatNumber(verifyRow.money)}**.`)
					}

					await app.player.removeMoney(message.author.id, scrapPrice)
					await app.player.addScrap(message.author.id, buyAmount)

					botMessage.edit(`Successfully traded **${app.common.formatNumber(scrapPrice)}** Racoin for **${app.common.formatNumber(buyAmount, false, true)}** Scrap.\n\nYou now have **${app.common.formatNumber(verifyRow.money - scrapPrice)}** Racoin and **${app.common.formatNumber(verifyRow.scrap + buyAmount, false, true)}** Scrap`)
				}
				else {
					botMessage.delete()
				}
			...
			const botMessage = await message.channel.createMessage(`Purchase ${listInfo.amount}x ${app.itemdata[listInfo.item].icon}\`${listInfo.item}\` for **${app.common.formatNumber(listInfo.price)}** Racoin?`)
			...
			try {
				...
				if (confirmed) {
					...
					if (row.money < listInfo.price) {
						return botMessage.edit(`❌ You don't have enough Racoin! You only have **${app.common.formatNumber(row.money)}**`)
					}
					if (!hasSpace) {
		...
		else if (args[0] && args[0].toLowerCase() === 'Racoin') {
			message.reply('You can\'t buy Racoin directly using Scrap, you should instead check the daily scrap deals in the `shop`!')
		}
		else {
