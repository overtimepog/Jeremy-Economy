			...
			if (confirmed) {
				...
				async function handleMsg(m, userArgs, command, player) {
					if (command.toLowerCase() === 'cancel') {
						...
						if (!amount) {
							return m.channel.createMessage(`❌ You should specify an amount of Racoin. You currently have **${app.common.formatNumber(row.money)}**`)
						}
						else if (row.money < amount) {
							return m.channel.createMessage(`❌ You don't have that much Racoin. You currently have **${app.common.formatNumber(row.money)}**`)
						}
